...
template <size_t Dim, Face Location>
...
template <typename Block, size_t Dim, Face Location>
auto padding_ptr(Block& block, FaceSpecifier<Dim, Location>)
  -> decltype(block.begin().data()) {
  if constexpr (Location == Face::start) {
    return block.begin().data();
  } else {
    constexpr auto dim           = Dimension<Dim>();
    const auto     padding_start = block.size(dim) - block.padding() - 1;
    return block.begin().offset(dim, padding_start).data();
  }
}
...
auto memcopy_padding(
  ...
  using allocator_t = typename block_traits_t<SrcBlock>::allocator_t;
...
auto memcopy_padding(
  const SrcBlock&         src_block,
  DstBlock&               dst_block,
  FaceSpecifier<Dim, Src> src_face,
  FaceSpecifier<Dim, Dst> dst_face) -> void {
  ...
  // If copying in x dimension, we just need to copy the padding with,
  ...
  // If copying in the y dimension is the opposite as above. We need to
