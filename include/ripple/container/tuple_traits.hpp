//==--- ripple/container/tuple_traits.hpp ------------------ -*- C++ -*- ---==//
//            
//                                Ripple
// 
...
/// The Tuple type holds a fixed-size number of heterogenous types. This 
/// implementation of a tuple is similar to std::tuple, but allows the tuple
...
/// also limiting.
///
/// Having out own implementation also allows for more functionality to be added
/// where required.
/// 
...
///
/// However, if the tuple does store reference types, then it will not work
...
struct TupleElement {
  ...
  using type_t = decltype(
    type_extractor<I>(std::move(std::declval<T>().data()))
  );
};
...
template <size_t I, typename T>
using tuple_element_t = typename detail::TupleElement<I, T>::type_t;

/// Returns true if a decayed T is a tuple, and false otherwise. This will decay
/// the type T first.
...
template <typename... Ts>
using tuple_enable_t = std::enable_if_t<
  is_tuple_v<nth_element_t<0, Ts...>>, int
>;

/// Defines a valid type if the type T is __not__ a tuple, otherwise does not
/// define a valid type.
/// \tparam T  The type to base the enable on.
/// \tparam Ts The rest of the types which do not effect the enable.
template <typename... Ts>
using non_tuple_enable_t = std::enable_if_t<
  !is_tuple_v<nth_element_t<0, Ts...>>, int
>;
