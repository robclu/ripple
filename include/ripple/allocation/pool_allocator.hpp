 ...
 * \tparam FreeList     The implementation type of the freelist.
 */
...
class PoolAllocator {
 ...
 public:
  ...
  /**
   * Constructor which initializes the freelist with the start and end
  ...
  /**
   * Moves constructor to move other into this allocator.
   * \param other The other allocator to move into this one.
   */
  PoolAllocator(PoolAllocator&& other) noexcept = default;

  /**
   * Move assignment operator to move other into this allocator.
   * \param other The other allocator to move into this one.
   */
  ...
  /**
   * Allocates an element of size with a given alignment from the pool.
   *
   * \note This will fail if size is larger than the element size for the pool
   *       or if the alignment is larger than the alignment for the pool.
   *
   * \note If the pool is full, this will return a nullptr.
   *
  ...
  /**
   * Frees the ptr, pushing it onto the front of the freelist.
  ...
  /**
   * Returns true if the allocator owns the ptr.
 ...
 private:
  FreeList    freelist_; //!< The freelist to allocate from and free into.
