...
TEST(storage_storage_traits, can_determine_layout_types) {
  ...
  auto b1 = ripple::is_storage_layout_v<ripple::StridedView>;
  auto b2 = ripple::is_storage_layout_v<ripple::ContiguousView>;
  auto b3 = ripple::is_storage_layout_v<int>;
  auto b4 = ripple::is_storage_layout_v<Test>;
  auto b5 = ripple::is_storage_layout_v<ripple::ContiguousOwned>;
...
template <typename T, typename U = ripple::ContiguousView>
struct Default {};
// Struct with int.
...
TEST(storage_storage_traits, can_determine_if_type_has_storage_layout) {
  using mult_2_t   = Params<int, ripple::ContiguousView>;
  using mult_3_t   = Params<ripple::StridedView, int, float>;
  using default_t  = Default<int>;
  using with_int_t = WithInt<4, float, ripple::StridedView>;
  using int_wrap_t = Params<ripple::Int64<4>, float, ripple::StridedView>;
...
TEST(storage_storage_traits, can_determine_storage_layout_type) {
  ...
  using default_t  = Default<int>;
  using with_int_t = WithInt<4, float, ripple::StridedView>;
  using int_wrap_t = Params<ripple::Int64<4>, ripple::ContiguousOwned>;
...
template <typename T, typename S = ripple::ContiguousOwned>
struct Test : ripple::StridableLayout<Test<T, S>> {
  using descriptor_t =
    ripple::StorageDescriptor<S, ripple::StorageElement<int, 3>, float>;
};

using test_strided_t = Test<int, ripple::StridedView>;
using test_contig_t  = Test<int, ripple::ContiguousView>;
using test_owned_t   = Test<int, ripple::ContiguousOwned>;
/*
